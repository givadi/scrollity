...
function slidesReducer(state: Array<Slide> = initialState.slides, action: SlideAction): Array<Slide> {
    switch (action.type) {
        ...
        case SlideActionTypes.MOVE_BLOCKS:
            return moveBlocks(state, action.payload.slideId, action.payload.blocksIds, action.payload.newPosition);
        case SlideActionTypes.RESIZE_BLOCKS:
            return resizeBlocks(state, action.payload.slideId, action.payload.blocksIds, action.payload.newSize);
